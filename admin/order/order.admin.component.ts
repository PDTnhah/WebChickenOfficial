...
export class OrderAdminComponent implements OnInit{  
  ...
  getAllOrders(keyword: string, page: number, limit: number) {
    this.orderService.getAllOrders(keyword, page, limit).subscribe({
      next: (response: any) => {
        // Cập nhật danh sách đơn hàng và các thuộc tính liên quan
      ...
      error: (error: any) => {
        ...
        // Thực hiện xử lý lỗi nếu cần, ví dụ thông báo cho người dùng
      }
    });
  }
    ...
    if (confirmation) {
      this.orderService.deleteOrder(id).subscribe({
        next: (response: any) => {
          ...
          this.toastr.success(`Xóa đơn hàng của ${response.fullname}`, 'Thành công');  
        },
        error: (error: any) => {
          debugger
